...
def get_prior_auth_output_model(
    fields: List[Dict[str, Any]], model_name: str = "PriorAuthOutputModel"
) -> Type[BaseModel]:
    """
    ...
    for f in fields:
        ...
        if f_type_str == "list" or f_type_str.startswith("list[") or f_type_str.startswith("List["):
            ...
            if sub_fields and isinstance(sub_fields, list):
                ...
                sub_model = get_prior_auth_output_model(sub_fields, model_name=sub_model_name)
    ...
    return create_model(model_name, __base__=PriorAuthBaseModel, **dyn_fields)